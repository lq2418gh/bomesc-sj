...
<body>
	<h3 align="center">管线专业变更信息图纸汇总</h3>
		...
		<div class="search_div">
			<div class="search_all_div">
				<div class="search_item">
					...
					<span class="ml50">
						<input id="update_date_search" name="update_date" class="easyui-datebox" data-options="width:150,editable:false" dkd-search-element="pit.update_date = text"/>
					</span>
				</div>
				<!--<div class="search_item">
	...
	<div style="margin-top:5px;width:100%;text-align:center">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-back" onclick="jumpPage('管线数据跟踪表','${ctx}/login/showList.do?url=business/piDesignDataManager/piDataTrackChange')">返回</a>
	</div>
    <div class="datagrid_div" style="padding-left:10px">
	    <table id="pi_data_change_collect">
			<thead>
				<tr>
					...
					<th data-options="field:'module_no',width:140">模块</th>
...
<script type="text/javascript">
	...
	var editIndex;
	$(function(){
		$("#pi_data_change_collect").datagrid({
			rownumbers:true,
			url:"${ctx}/piDesignDataChange/findByProfessional.do",
	        ...
	        onExpandRow: function(index,row){
	          ...
	          ddv.datagrid({
	            url:'${ctx}/piDesignDataChange/findChangeDetails.do?project_name='+row.project_name+'&shop_draw_no='+row.shop_draw_no+'&module_no='+row.module_no+'&update_date='+row.update_date,
	            fitColumns:true,
	            rownumbers:true,
	            loadMsg:'',
	            height:'auto',
	            columns:[[
	              ...
	              {field:'module_no',title:'模块',width:90,align:'center'},
	            ...
	            onResize:function(){
	              $('#pi_data_change_collect').datagrid('fixDetailRowHeight',index);
	            },
    ...
    function endEditing(){
        ...
        if ($("#pi_data_change_collect").datagrid("validateRow", editIndex)){
            $("#pi_data_change_collect").datagrid("endEdit", editIndex);
            editIndex = undefined;
            return true;
        } else {
            return false;
    ...
    function editReason(){
    	if(isEmpty(clickIndex)&&0!=clickIndex){
    	    ...
    	    if (editIndex != clickIndex){
    	        if (endEditing()){
    	        	 $("#pi_data_change_collect").datagrid("beginEdit", clickIndex);
    ...
    window.onload = function() {
 	...
 	function searchDataChange(){
 		var updateDate=$("#update_date_search").datebox("getValue");
 		var project_name=$("#project_name_search").textbox("getValue");
 		$("#pi_data_change_collect").datagrid({
 	...
 	function save(){
 		...
 		if(endEditing()){
 	 		var rows = $('#pi_data_change_collect').datagrid('getRows');
 	 		...
 	 		if(isEmpty(reason_for_change)||"请选择"==reason_for_change){
 				...
 				$("#pi_data_change_collect").datagrid('reload');
 	 		}else{
				delete row.professional;
 	 			$.ajax({
 	 	 			type:"post",
 	 	 			url:"${ctx}/piDesignDataChange/saveReason.do",
