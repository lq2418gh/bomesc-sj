...
<body>
	<form class="search_form dkd-search">
		<div class="search_div">
			<div class="search_all_div">
				...
				<div class="search_item" ignore>
					<label class="w100">标记类型</label>
					<span class="ml50">
						<input id="cancel" type="radio" name="markType" value="0" checked>升版作废</input>
                		<input id="add" type="radio" name="markType" value="1">升版增加</input>
					</span>
				</div>
			</div>
			<div style="margin-top:5px;width:100%;text-align:center">
				...
				<a id="cancelButton" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-save" onclick="changeState('作废')">作废</a>
				<a id="addButton" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-save" onclick="changeState('删除')">删除</a>
			</div>
		</div>
	</form>
	<!-- <div id="tab" class="easyui-tabs" style="width:22%;height:30px;margin-left:10px;">
	    <div title="图纸作废标记" style="padding:20px;display:none;">
	    	
	    </div>
	    <div title="图纸升版增加标记" data-options="closable:false" style="overflow:auto;padding:20px;display:none;">
			
	    </div>
	</div> -->
	<div class="datagrid_div" id="st_data_track_div">
		<table id="st_data_mark" align="center" style="width:100%;">
			<thead>
				<tr>
					<th field="ck" checkbox="true"></th>
					<th data-options="field:'state_change_date',width:75,halign:'center',sortable:true">提示时间</th>
					<th data-options="field:'warn_type',width:75,halign:'center',sortable:true">提示类型</th>
					<th data-options="field:'mark_date',width:75,halign:'center',sortable:true">标记日期</th>
					<th data-options="field:'mark_user',width:75,halign:'center',sortable:true">标记人</th>
					...
					<th data-options="field:'change_size',width:75,halign:'center'">变更次数</th>
    ...
    var currentRow,YN=[{"text":"是","value":"Y"},{"text":"否","value":"N"}],columns={},project="",init=true,isProjectChange=true,module="cancel";
 	$(function(){
 		$("#st_data_mark").datagrid({
			rownumbers:true,
			pagination:true,
 		...
 		$("#st_data_mark").datagrid("loadData",{total:0,rows:[]});
 	...
 	function getColumns(){
 		...
 		if(!isEmpty(data[0])){
    		...
    		for(var i=0;i<showColumns.length;i++){
        		$("#st_data_mark").datagrid("showColumn",showColumns[i].substring(4,showColumns[i].length));
       		}
    		$("#st_data_mark").datagrid("hideColumn","update_date");
    		$("#st_data_mark").datagrid("hideColumn","change_size");
    	}else{
    		...
    		for(var i=0;i<allColumns.length;i++){
    			$("#st_data_mark").datagrid("showColumn",allColumns[i]);
    		}
    	}
 	}
 	...
 	function searchDesignDataManager(){
 		...
 		if("cancel"==module){
 			...
 			condition = {"added_by_drawing_update":"Y","state":"新增"};
 		}
 		$("#st_data_mark").datagrid({
 		    url:"${ctx}/stDesignDataManager/queryForMark.do",
 	...
 	function changeState(type){
 		var currentRows =  $('#st_data_mark').datagrid('getChecked');
 		...
 		for(var i=0;i<currentRows.length;i++){
 			ids+=","+currentRows[i].id;
 		}
 		ids = ids.substring(1,ids.length);
 		$.ajax({
 			type:"post",
 			url:"${ctx}/stDesignDataManager/saveState.do",
 			contentType:"application/json;charset=utf-8",
 			async:false,
 			data:JSON.stringify({"ids":ids,"state":type}),
 			success:function(result){
 				handleReturn(result,function(){
 					searchDesignDataManager();
 				});
 			}
 		})
 	}
 	...
 	function cancelModule(){
 		...
 		$("#cancelButton").show();
 		$("#st_data_mark").datagrid("loadData",{total:0,rows:[]});
 		$("#st_data_mark").datagrid("showColumn","ck");
 	}
 	//点击升版增加按钮触发事件
 	function addModule(){
 		module="add";
 		...
 		$("#cancelButton").hide();
 		$("#st_data_mark").datagrid("loadData",{total:0,rows:[]});
 		$("#st_data_mark").datagrid("hideColumn","ck");
 		$("#st_data_mark").datagrid("hideColumn","mark_user");
 		$("#st_data_mark").datagrid("hideColumn","mark_date");
 	}
</script>
</body>
</html>