		...
		<table id="record_data" align="center" style="width:100%">
			<thead>
				<tr>
					...
					<th data-options="field:'email',width:150,align:'center'" >发送邮箱</th>
					<th data-options="field:'file_path',width:200,align:'center'" >失败文件名称</th>
					...
					<th data-options="field:'module',width:100,align:'center'">模块</th>
					<th data-options="field:'prompt_content',width:100,align:'center',hidden:'true'">提示内容</th>
				</tr>
			</thead>
		</table>
	</div>
	<div id="prompt_content_view" class="easyui-dialog pop" style="width:450;height:250;top:100px;padding:10px 20px;overflow:auto;" closed="true" buttons="#dlg-failInfo-buttons" maximizable=true modal=true>
    ...
    var currentRow;
 	$(function(){
 		$("#record_data").datagrid({
			...
			singleSelect:true,
			pagination:true,
            url:"${ctx}/importFailure/recordList.do",
 	...
 	//点击一行时，将数据加载到弹出框中
 	/**function insertData(rowData){
 		var contents = rowData.prompt_content.split(" ");
 		var htmlText="";
 		for(var i=0;i<contents.length;i++){
 			htmlText+="<tr><td>"+(i+1)+"</td><td>"+contents[i]+"</td><tr/>"
 		}
 		$("#importInfo").html(htmlText);
 	}*/
 	//时间比较
